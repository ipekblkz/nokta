...
const EXPERT_SYSTEM = `Sen Ahmet Yılmaz'sın. 15 yıllık deneyime sahip bir girişim danışmanı ve çeviricisin. NOKTA platformunda çalışıyorsun.
Kullanıcının fikri: {IDEA}
Spec: {SPEC}
Kısa, samimi ve pratik cevaplar ver. Türkçe konuş. Emoji kullanabilirsin.`;
...
function DotCapture() {
  ...
  const openExpertChat = () => {
    setChatMessages([{
      role: 'assistant',
      content: `Merhaba! Ben Ahmet Yılmaz, NOKTA'nın girişim danışmanıyım. 15 yıldır startup ekosisteminde çalışıyorum.\n\nFikrin veya girişim sürecinle ilgili her şeyi sorabilirsin.`,
    }]);
  ...
  const handleSendChat = useCallback(async () => {
    ...
    try {
      const systemPrompt = EXPERT_SYSTEM
        .replace('{IDEA}', rawIdea || 'belirtilmedi')
        .replace('{SPEC}', spec || 'henüz oluşturulmadı');
      const reply = await callGemini(systemPrompt, updated);
  ...
  return (
    <View style={{ flex: 1 }}>
      {/* Expert Chat — tam ekran, ayrı layout */}
      {phase === 'expert' ? (
        <View style={[styles.expertRoot, { paddingTop: insets.top }]}>
          {/* Header */}
          <View style={styles.expertHeader}>
            <TouchableOpacity
              style={styles.expertBackBtn}
              onPress={() => fadeTransition(() => setPhase(spec ? 'spec' : 'input'))}
            >
              <Text style={styles.expertBackText}>←</Text>
            </TouchableOpacity>
            <View style={styles.expertAvatar}>
              <Text style={styles.expertAvatarText}>AY</Text>
            </View>
            <View style={styles.expertHeaderInfo}>
              <Text style={styles.expertName}>Ahmet Yılmaz</Text>
              <Text style={styles.expertTitle}>Girişim Danışmanı • Çevirici</Text>
            </View>
          </View>
          ...
          <ScrollView
            ...
            contentContainerStyle={styles.expertScrollContent}
            keyboardShouldPersistTaps="handled"
          >
            {chatMessages.map((msg, i) => (
              <View key={i} style={msg.role === 'user' ? styles.expertMsgRowUser : styles.expertMsgRowAgent}>
                {msg.role === 'assistant' && (
                  <View style={styles.expertMsgAvatar}>
                    <Text style={styles.expertMsgAvatarText}>AY</Text>
                  </View>
                )}
                <View style={[styles.expertBubble, msg.role === 'user' ? styles.expertBubbleUser : styles.expertBubbleAgent]}>
                  {msg.role === 'assistant' && <Text style={styles.expertBubbleName}>Ahmet Y.</Text>}
                  <Text style={[styles.expertBubbleText, msg.role === 'user' && { color: '#fff' }]}>{msg.content}</Text>
                </View>
              </View>
            ))}
            {chatLoading && (
              <View style={styles.expertMsgRowAgent}>
                <View style={styles.expertMsgAvatar}>
                  <Text style={styles.expertMsgAvatarText}>AY</Text>
                </View>
                <View style={styles.expertBubbleAgent}>
          ...
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
            <View style={[styles.expertInputRow, { paddingBottom: insets.bottom + 8 }]}>
              <TextInput
              ...
              />
              <TouchableOpacity
                style={[styles.expertSendBtn, !chatInput.trim() && styles.btnDisabled]}
                onPress={handleSendChat}
                disabled={!chatInput.trim() || chatLoading}
              >
                <Text style={styles.expertSendText}>→</Text>
              </TouchableOpacity>
            </View>
...
const styles = StyleSheet.create({
  ...
  // Expert Chat Styles
  expertRoot: { flex: 1, backgroundColor: '#f0faf0' },
  expertHeader: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', paddingHorizontal: 16, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#e8f5e9', elevation: 2 },
  expertBackBtn: { marginRight: 12, padding: 4 },
  expertBackText: { fontSize: 22, color: '#2d6a4f', fontWeight: '700' },
  expertAvatar: { width: 42, height: 42, borderRadius: 21, backgroundColor: '#4caf50', alignItems: 'center', justifyContent: 'center', marginRight: 12 },
  expertAvatarText: { color: '#fff', fontWeight: '800', fontSize: 14 },
  expertHeaderInfo: { flex: 1 },
  expertName: { fontSize: 16, fontWeight: '800', color: '#1a3c2a' },
  expertTitle: { fontSize: 12, color: '#888', marginTop: 1 },
  expertScroll: { flex: 1 },
  expertScrollContent: { padding: 16, paddingBottom: 8 },
  expertMsgRowAgent: { flexDirection: 'row', alignItems: 'flex-end', marginBottom: 12 },
  expertMsgRowUser: { flexDirection: 'row', justifyContent: 'flex-end', marginBottom: 12 },
  expertMsgAvatar: { width: 30, height: 30, borderRadius: 15, backgroundColor: '#4caf50', alignItems: 'center', justifyContent: 'center', marginRight: 8 },
  expertMsgAvatarText: { color: '#fff', fontWeight: '800', fontSize: 10 },
  expertBubble: { maxWidth: '78%', borderRadius: 16, padding: 12 },
  expertBubbleAgent: { maxWidth: '78%', backgroundColor: '#fff', borderRadius: 16, padding: 12, elevation: 1 },
  expertBubbleUser: { backgroundColor: '#4caf50' },
  expertBubbleName: { fontSize: 10, color: '#4caf50', fontWeight: '800', marginBottom: 4 },
  expertBubbleText: { fontSize: 14, lineHeight: 22, color: '#333' },
  expertInputRow: { flexDirection: 'row', alignItems: 'flex-end', gap: 10, paddingHorizontal: 16, paddingTop: 10, backgroundColor: '#fff', borderTopWidth: 1, borderTopColor: '#e8f5e9' },
  expertInput: { flex: 1, backgroundColor: '#f0faf0', borderRadius: 22, paddingHorizontal: 16, paddingVertical: 10, color: '#333', fontSize: 14, maxHeight: 100 },
  expertSendBtn: { width: 44, height: 44, borderRadius: 22, backgroundColor: '#4caf50', alignItems: 'center', justifyContent: 'center' },
  expertSendText: { color: '#fff', fontSize: 20, fontWeight: '800' },
});